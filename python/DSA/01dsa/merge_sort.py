"""
The time complexity of merge sort is  ( O(nlog n)), where  ( n  ) is the number of elements in the array to be sorted.

Merge sort achieves this time complexity because it divides the array into halves recursively until each subarray contains only one element. Then, it merges these subarrays back together in sorted order. 

The recursive division of the array takes  ( O( log n)  ) time since the array is repeatedly halved, resulting in  (  log n  ) levels of recursion. 

At each level of recursion, the merge operation takes  ( O(n)  ) time because it needs to merge two sorted subarrays of size  ( n/2  ) each into a single sorted array of size  ( n  ). 

Therefore, the total time complexity of merge sort is the product of the time complexity of the recursive division ( ( O( log n)  )) and the merge operation ( ( O(n)  )), which results in  ( O(n  log n)  ).
"""
